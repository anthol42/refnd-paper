#!/usr/bin/env python3
"""Train Linear + MLP heads on GUE DNA embeddings."""
import os
import argparse
import sys
...
from common import (
    DATASET_CONFIG, DNA_DATASETS, load_split,
    train_and_evaluate, save_seed_result,
)

BASE = Path(os.environ.get("RELAG_EXP_BASE", str(Path(__file__).resolve().parent.parent)))
DNA_METHODS = ["refnd", "random", "hestia", "datasail"]


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--method", choices=DNA_METHODS, required=True)
    ...
    df = pd.read_csv(Path(args.data_dir) / "dna" / args.dataset / "pooled.csv")
    ...
    save_seed_result(args.results_dir, "dna", args.dataset, args.method, args.seed, scores)
