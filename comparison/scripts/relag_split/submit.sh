#!/bin/bash
#SBATCH --job-name=relag_split
#SBATCH --account=def-sgobeil
#SBATCH --time=10:00:00
#SBATCH --cpus-per-task=16
#SBATCH --mem=64G
#SBATCH --output=logs/splits/relag_%j.out
...
echo "=== relag protein splits ==="
for ds in dbaasp_amp enzyme_topt; do
    echo "  -> $ds"
    python relag_split/run_protein.py --splits-dir "$SPLITS" --dataset "$ds"
done

echo "=== relag molecule splits ==="
for ds in cyp2c19_veith caco2_wang pgp_broccatelli ames lipophilicity sr_are; do
    echo "  -> $ds"
    python relag_split/run_molecule.py --splits-dir "$SPLITS" --dataset "$ds"
done

echo "=== relag DNA splits ==="
